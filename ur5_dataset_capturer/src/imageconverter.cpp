...
//Constructor
ImageConverter::ImageConverter(string fn1) : it_(nh_) { 
  filename = fn1.c_str();
}
...
void ImageConverter::imageCb(const sensor_msgs::ImageConstPtr &msg) {
  ...
  if(n == 0){
    while (!file_exists(filename.c_str()) && !working) {
      ...
      try {
        if(filename.compare(1, 4, "home") == 0){  //todo:filename were overlaped by GOD 
    ...
    printf("ImageConverter:wrong format:%s %s\n",filename.c_str(),filename.substr(l-4).c_str());
  }
  // cv::destroyAllWindows();
}
//member function  call set_filename to set imagefilename and begin subcribe topic
void ImageConverter::set_filename(string fn, std::string imageTopic) {
  filename = fn;
  working = false;
  image_sub_ = it_.subscribe(imageTopic.c_str(), 1, &ImageConverter::imageCb, this);
}


//==================================================================
//class name：  ImageConverted
 //author： Ning Chuanfang
 //date： 2020-04-22
...
//Constructor
ImageConverted::ImageConverted(string fn1) : it_(nh_) { 
  filename = fn1.c_str();                               
}
...
//callback function copy image topic into opencv and write it into file
void ImageConverted::imageCbDepth(const sensor_msgs::ImageConstPtr &msg) {
  int l,n = 1;
  l = filename.length();
  ...
  if(n == 0){
    ...
    while (!file_exists(filename.c_str()) && !working) {
      working = true;
      try {
        if(filename.compare(1, 4, "home") == 0){  //todo : find the bug filename overlay
          cv::imwrite(filename.c_str(), mono8_img); //write file 
          working = false;
        } else {
          printf("d filename is wrong\n");
        }
      } catch (runtime_error &ex) {
        printf("ImageConverted fn=%s %s %s\n",filename.c_str(),filename.substr(1,4).c_str(),ex.what());
        return;
      }
    }
  }else{
    printf("ImageConverted wrong file type:%s\n",filename.c_str());
  }
}

//member function  call set_filename to set imagefilename and begin subcribe topic
void ImageConverted::set_filename(string fn, std::string imageTopic) {
  filename = fn;
  working = false;
  image_sub_ = it_.subscribe(imageTopic.c_str(), 1, &ImageConverted::imageCbDepth, this);
}
